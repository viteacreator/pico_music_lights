...
9. Valid flash-region reporting validation. Preconditions: standard valid Release build only; intentionally overlapping configurations are build/link-time negative tests and must never be flashed to hardware. Action: boot and inspect bounded diagnostics or generated validation report. Expected diagnostics: application image bounds and persistent-region bounds are present, sector-aligned, within physical flash, and non-overlapping. Expected LEDs: normal saved-or-factory profile operation.
