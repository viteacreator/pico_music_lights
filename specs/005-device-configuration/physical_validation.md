...
1. Boot-load validation. Preconditions: Release build; both persistent slots erased by a known flashing or diagnostic erase step. Action: boot the device. Expected diagnostics: boot source `factory_defaults`, both slots erased or invalid, `has_persisted_record=false`, factory comparison baseline, active/draft factory defaults, dirty false, and no save in progress. Exercise Reload/discard and confirm the same no-record factory state remains. Then explicitly Save unchanged factory defaults and reboot; expected diagnostics show a valid slot A record at sequence 0, `has_persisted_record=true`, and dirty false. Expected LEDs: current six installed channels show factory Gyver VU output using the factory layout after audio starts; reserved channels 6 and 7 remain disabled in diagnostics; no automatic temporary diagnostic-scene cycling.
...
5. Representative save power-interruption validation. Preconditions: one previous valid slot. Action: perform repeated owner-controlled power removals during Save, using diagnostic injection pauses when available at representative payload-program and commit-page phases, including after commit-page programming begins and before final verification reports. Expected diagnostics after reboot: fresh independent slot inspection selects either the previous valid slot or a fully committed newer slot; an erased, partial, malformed, or inconsistent target is rejected. A same-runtime injected final-read failure after commit begins may report `commit_state_unknown`; it must preserve the in-RAM last-verified value, keep dirty true, and avoid erasing either slot, without claiming which record reboot will select. Expected LEDs after reboot: selected profile matches diagnostics. Exhaustive interruption at every byte or internal position of a 256-byte flash program operation and exhaustive valid/invalid/unknown read-failure classification remain host fake-flash requirements, not required owner hardware procedures. Exact internal hardware interruption testing is optional when dedicated external hardware supports it.
