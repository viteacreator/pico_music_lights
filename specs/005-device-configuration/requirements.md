...
R005-003. The configuration subsystem shall represent four separated configuration values: factory defaults, currently active runtime configuration, draft or pending configuration, and the last-verified-persisted comparison baseline. Persistent-record presence shall be represented separately by `has_persisted_record` or an equivalent boolean state and shall never be inferred from configuration-value equality.
...
R005-005. The currently active runtime configuration shall be the only configuration consumed by rendering, LED output, Idle Lighting, and Gyver effect-threshold rendering during normal operation.
...
R005-007. The subsystem shall track whether the draft or active preview differs from its logical comparison baseline. When `has_persisted_record` is true, that baseline is the last verified persisted configuration; when false, it is factory defaults. Empty-storage boot shall set `has_persisted_record=false`, active and draft to factory defaults, the comparison baseline to factory defaults, and dirty to false, while bounded boot diagnostics report factory fallback and no persisted record. An explicit Save of an otherwise unchanged factory-default draft shall remain permitted while `has_persisted_record=false`; it shall create the first committed record in slot A with sequence `0`, set `has_persisted_record=true` after full verification, and leave dirty false.

R005-008. Reload or discard shall restore the draft and previewable active values from the last verified in-RAM persisted configuration when `has_persisted_record=true`, or from factory defaults while `has_persisted_record=false`. In the latter case, presence remains false and dirty becomes false after successful runtime publication.
...
R005-015. The persisted profile shall include eight effect records, one per persistent logical LED channel. Current runtime adapters shall publish only board-supported channels 0 through 5 to the current six-channel LED driver and EffectEngine; the specification shall not claim that current runtime code already supports eight active channels or that the current LED transport can drive channels 6 and 7.
...
R005-018. The persisted profile shall include canonical device-level Gyver audio calibration values currently required for correct runtime effect rendering: Gyver VU left and right noise floors, Gyver VU hysteresis, Gyver spectrum noise floor, and Gyver spectrum minimum peak. These values shall be owned only by the global audio-calibration record, serialized exactly once, excluded from per-channel persisted effect records, overlaid into applicable Gyver runtime effect configurations during publication, consumed by the EffectEngine/effect renderer, applied atomically at the existing render boundary, and used as the only source for dirty-state comparison, equality, factory defaults, codec round trips, and schema tests. Feature 005 shall not require or design a new audio-processing calibration publication path for these Gyver effect thresholds. The audio safe point is required for controlled flash Save/reset operations, not for ordinary preview publication of Gyver effect thresholds. Conflicting duplicated values from runtime `effects::StripEffectConfig` instances shall be normalized from the global calibration and shall not enter the persisted payload. Idle Lighting activity floors and hysteresis shall remain owned by `effects::IdleLightingConfig` and shall not be duplicated in `AudioCalibrationConfig`.
...
R005-021. Factory LED channel order shall be GRBW for all eight persistent channels, including disabled reserved channels 6 and 7.

R005-022. Factory LED reversal shall be false for all eight persistent channels.

R005-023. Factory LED brightness shall be exactly 16 out of 255 for all eight persistent channels, including disabled reserved channels 6 and 7.
...
R005-025. The current six independent Gyver VU effect configurations shall remain the factory effect defaults for channels 0 through 5. Reserved channels 6 and 7 shall default exactly to disabled, zero pixels, Effect Off, GRBW channel order, brightness 16, not reversed, `length_mm=0`, and `density_pixels_per_metre=0`.
...
R005-029. Effect parameters, colours, brightness, Idle Lighting settings, and safe Gyver effect-threshold calibration values may be previewed live after field-level validation at the render boundary.
...
R005-034. Validation shall require every logical channel index to be within the fixed eight-channel persistent capacity. It shall reject enabling a channel that is not supported by the current board configuration. Feature 005 shall not implement runtime or hardware support for reserved channels 6 and 7. Future activation of channels 6 and 7 is a separate approved hardware/runtime task and shall include an explicit PIO/DMA resource design compatible with Pico W networking. Future board support may increase the supported channel count from six to eight without changing the Feature 005 persistent schema or requiring migration solely for the planned channels.
...
R005-036. Validation shall reject total enabled board-supported pixels above the current board/runtime total-pixel capability, currently `board::kMaxConfiguredPixels` (800). Total calculations shall use at least `uint32_t`. The persistent representation maximum for per-channel `pixel_count` is `UINT16_MAX`; the persistent codec can represent eight channels at that representation maximum. Board per-channel and total-pixel capabilities are not part of the persistent schema, and a future board may change either capability without changing schema 1. Disabled and unsupported channels shall not contribute to the total. When adapting a disabled channel to the current LED runtime, the adapter shall publish `enabled=false`, runtime `pixel_count=0`, no pixel-buffer slice, and no transmission while leaving the retained persistent `pixel_count` unchanged.
...
R005-047. The implementation shall not persist raw C++ structure memory with `memcpy` or equivalent object-representation copying. The canonical device model shall use schema-owned configuration structs for effects and Idle Lighting; runtime `effects::StripEffectConfig`, runtime `effects::IdleLightingConfig`, and their C++ padding or runtime-only fields shall not be persisted or used as schema storage.
...
R005-050. Each schema-1 slot record shall use the exact binary layout in `design.md`: a 64-byte record header, a 978-byte schema-1 payload, erased data-region padding, and a dedicated final 256-byte commit page. The header shall include exact record magic bytes, `record_format_version`, `payload_schema_version`, payload length, monotonic sequence, payload CRC32, slot geometry fields, and header CRC32. Schema 1 payload shall not duplicate the major schema version and shall start with two little-endian `uint16_t` control fields, `payload_schema_minor` at bytes 0..1 and `payload_flags_reserved` at bytes 2..3, both zero for schema 1.
...
R005-052. A newly written slot shall not become authoritative until its complete header, payload, CRC metadata, and exact commit-page contents are valid. No requirement shall assume that a four-byte flash program operation is available.

R005-053. After interrupted erase or programming, normal slot inspection shall recover the previous valid record, a fully committed newer record, or safe factory defaults when neither slot validates; no interrupted phase shall require interpreting a partial record.
...
R005-056. If both slots are invalid, missing, erased, unsupported, or corrupt, loading shall use factory defaults and expose the fallback reason through bounded diagnostics. This boot state shall have `has_persisted_record=false`, factory defaults as the comparison baseline, active and draft equal to factory defaults, and dirty false.
...
R005-058. Save and factory reset shall use deterministic write-target selection that never erases the selected valid slot while preparing its replacement. When one valid selected slot exists, the target shall be the other slot; when both slots are valid, the target shall be the non-selected slot; when neither slot is valid, the first attempted write shall target slot A with initial sequence `0`; otherwise the next sequence shall be `(selected_sequence + 1) mod 2^32`. The exact transaction shall erase the target, program header and payload data pages while the final 256-byte commit page remains erased, verify the uncommitted header and payload, program the complete commit page as the final authority-changing flash operation, and then read back/decode/CRC-check/validate the committed record. Before commit-page programming begins, any previously selected valid slot is definitely authoritative; with empty storage, no physical record is authoritative. Once commit-page programming has been attempted, the target may be fully valid, partially programmed, or invalid; no program, readback, verification, reset, or power failure in that phase proves that an old slot remains physically authoritative. When reads remain available, the implementation shall best-effort inspect both slots: a completely valid target selected as newest finalizes success; a conclusively invalid target leaves any previous selected valid slot authoritative, or leaves storage without a valid record when none existed; and an indeterminate result shall report typed bounded `commit_state_unknown`, leave the in-RAM last-verified-persisted value and `has_persisted_record` unchanged, keep dirty true, and erase neither slot automatically. After a conclusively invalid first Save/reset from empty storage, presence remains false, the factory comparison baseline remains unchanged, and the next boot falls back to factory defaults. The next boot after an unknown result shall independently inspect both slots and may select either valid record under normal validity and sequence rules. Factory reset shall use the same transaction and shall not erase both slots.
...
R005-062. The controlled save/reset sequence shall handle audio capture and processing deliberately during flash operations, either by pausing and accounting for dropped work or by proving that the selected flash backend is safe for the active capture mode. Audio-safe boundary acquisition shall use a finite compile-time bounded deadline; timeout shall abort before activation and before any flash erase/program/commit operation, restore runtime operation, leave active, draft, `has_persisted_record`, and the in-RAM last-verified-persisted value unchanged except bounded diagnostics, and report a typed audio-safe-point timeout. Save shall perform validation/canonicalization, fixed-buffer serialization, and read-only slot inspection before safe-point acquisition without changing active runtime state. Structural activation, if required, shall occur only after both safe points are acquired; activation failure shall issue no erase/program/commit. Flash or verification failure after successful activation may leave the new configuration active while the in-RAM last-verified-persisted value remains unchanged and dirty remains true; physical authority after commit-page programming begins shall follow R005-058, and Reload/discard is the explicit same-runtime rollback mechanism.
...
R005-067. Host tests shall cover positive, boundary, disabled-channel runtime adaptation with retained persistent pixel count, repeated structural enable/disable without resource leak, PIO/DMA claim failure rollback, corruption, truncation, sequence-wrap, exact half-range ambiguity, duplicate sequence, unsupported-schema, both-slots-invalid, empty-flash boot presence/baseline/active/draft/dirty diagnostics, empty-storage Reload/discard, explicit Save of unchanged factory defaults to slot A sequence 0, interrupted-erase, interrupted-program, commit completed followed by final-readback failure, post-commit retry inspection finding the target conclusively valid, conclusively invalid, or unknown because reads fail, reboot selection after each post-commit outcome, old-slot preservation, unknown-case dirty and diagnostic semantics, CRC mismatch, length mismatch, malicious-field, eight-channel schema records, current six-channel board capability, reserved channels 6 and 7 disabled, rejection of unsupported enabled channels, absence of persisted GPIO, current runtime adapters consuming only board-supported channels, total-pixel capability outside the schema, 300 pixels accepted on the current board, 301 pixels rejected on an enabled current-board channel, 500 pixels rejected even when total enabled pixels remain below 800, total 800 accepted, total 801 rejected, disabled reserved channel retaining a schema-valid count without runtime publication, valid current Idle masks, current-board rejection of Idle bits 6 and 7, eight-bit Idle mask codec round trips, canonical Gyver calibration ownership, runtime calibration overlay, serialization without duplicated calibration values, prevention of contradictory calibration state, known/unknown millimetre physical metadata, golden header bytes, golden LED record bytes, golden effect record bytes, golden Idle record bytes, golden audio record bytes, exact complete payload length, exact slot data capacity, CRC-32/ISO-HDLC check vector, header CRC coverage mutation tests, payload CRC coverage mutation tests, erased commit page, fully valid commit page, interruption throughout the 256-byte commit-page program, nonzero reserved bytes, unknown enum ids, payload padding excluded from CRC, commit page excluded from header and payload CRC, no C++ padding or raw structure representation entering persistent bytes, deterministic write-target selection, preservation of the selected slot, initial save, alternating slots, write failure, readback failure, reset failure, bounded LED/audio safe-point timeout recovery, proof that no erase, program, or commit operation occurs before both safe points, and flash-overlap cases.
