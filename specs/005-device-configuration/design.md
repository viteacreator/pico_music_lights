...
- Last-verified-persisted comparison baseline: owned by `ConfigService`; paired with a separate `has_persisted_record` boolean. The value is updated only after a save or factory-reset target is fully read back and selected as valid, or after boot loads a valid supported record. Presence is never inferred by comparing values.

On empty-storage boot, slot inspection establishes no valid supported record, `has_persisted_record=false`, the comparison baseline is factory defaults, active and draft are factory defaults, and dirty is false. Boot diagnostics report both factory fallback and no persisted record. Dirty state is true when draft or active preview differs from the comparison baseline. Equality is deterministic field-by-field comparison, not byte comparison of structs. Equality and dirty-state checks operate on schema-owned `config::EffectDeviceConfig` and `config::IdleLightingDeviceConfig` values, not runtime effect/Idle structs. They ignore duplicated Gyver calibration fields inside runtime `StripEffectConfig` mirrors and compare only `DeviceConfiguration::audio_calibration` for those values. Presence false does not itself make the configuration dirty and does not prohibit Save: explicitly saving unchanged factory defaults creates the first committed record in slot A with sequence 0; only full committed-record verification changes presence to true.
...
Save follows one ordered state machine: (1) validate and canonicalize the draft without changing active runtime state; (2) serialize the canonical payload into a fixed RAM buffer without changing active runtime state; (3) inspect slots and choose the target using safe read-only operations; (4) acquire the bounded LED safe point; (5) acquire the bounded audio safe point; (6) perform required controlled activation only after both safe points are acquired; (7) erase the target slot; (8) program header and payload data pages while the commit page remains erased; (9) read back and verify the uncommitted header and payload; (10) program the complete 256-byte commit page; (11) read back and perform full committed-record validation, including normal two-slot selection; (12) update the last-verified-persisted value and presence and clear dirty only after complete validation selects the target; and (13) restore audio/rendering and finalize diagnostics. LED or audio safe-point timeout occurs before activation and before erase/program/commit, leaves active, draft, presence, and last-verified state exactly as they were before Save except diagnostics, and preserves dirty state. Activation failure issues no erase/program/commit and leaves active and last-verified state unchanged. A failure before commit-page programming leaves the target uncommitted; any previously selected valid slot remains definitely authoritative, while an empty store remains without a physically authoritative record.

Once programming of the commit page has been attempted, physical authority is uncertain until both slots can be inspected: the target can be fully valid, partially programmed, or invalid. A commit-program failure, final-readback failure, verification failure, reset, or power interruption after that point never proves that an old slot is still physically authoritative. If reads remain available, Save performs a best-effort normal inspection of both slots. If the target validates completely and is selected as newest, Save may finalize successfully, update the in-RAM last-verified value and presence, and clear dirty. If the target is conclusively invalid, any previous selected valid slot remains authoritative; if none existed, storage remains without a valid record, `has_persisted_record` stays false, and the factory baseline remains unchanged. In either invalid case, the in-RAM last-verified state remains unchanged and dirty remains true if activation changed active/draft. If validity or selection cannot be determined because reads fail or inspection is otherwise inconclusive, diagnostics report bounded typed `commit_state_unknown`, the in-RAM last-verified value and `has_persisted_record` remain unchanged, dirty remains true, and neither slot is erased or repaired automatically. On the next boot, ordinary independent two-slot inspection may select the previous or new record according to exact validity and sequence rules, or fall back to factory defaults if neither validates.

Reload/discard replaces draft with the in-RAM last-verified-persisted value when presence is true, or factory defaults while presence is false. If preview had changed active non-structural values, reload republishes the restored values at a safe boundary; structural rollback follows the same controlled reinitialization/restart rule. In a `commit_state_unknown` runtime, Reload/discard restores that in-RAM value and clears dirty only after successful rollback publication, but diagnostics must retain the uncertainty and must not claim that this proves which physical record the next boot will select.

Factory reset requires a confirmation token or equivalent explicit two-step API that cannot be triggered by malformed draft data. Confirmed reset uses the same ordered transaction with factory defaults as the canonical configuration: construct and serialize defaults, inspect slots, acquire LED and audio safe points, activate defaults only after both safe points, erase the target slot, program header and payload while the commit page remains erased, verify the uncommitted data region, program the complete commit page, and then verify and select the committed record. Safe-point timeout or activation failure issues no erase/program/commit. Before commit-page programming, failure preserves the previously selected authority. After commit-page programming is attempted, reset applies the same valid/invalid/unknown best-effort two-slot inspection and in-RAM state rules as Save; factory defaults may remain active while the last-verified-persisted value remains old and dirty remains true. Diagnostics must distinguish persisted reset success, conclusive target invalidity, and `commit_state_unknown`.
...
Any old selected slot remains untouched while preparing the target. Interrupted erase of the target can destroy only that target; interrupted programming before the commit page leaves the target uncommitted, so any previously selected valid slot remains definitely authoritative, while a first write from empty storage leaves no valid record. Once commit-page programming is attempted, the target may become a valid newer authority even if the operation reports failure or final readback fails. Authority is then classified only by best-effort normal inspection of both slots as target valid and selected, target conclusively invalid, or unknown; an old slot, when present, is preserved in every case but its preservation alone does not prove which record is authoritative. If inspection is unavailable or inconclusive, no automatic erase occurs and next boot performs normal independent selection. If both slots are conclusively invalid, factory defaults are used and the fallback reason reports both slot states.
...
Diagnostics are fixed-size summaries: boot source, fallback reason, persisted-record presence, slot A/B validity and sequence, selected sequence, commit-state classification including `commit_state_unknown`, dirty flag, last validation error, last save status, active-differs-from-last-verified status, last save duration, interrupted audio blocks or dropped LED frames during save, factory-reset status, schema status, flash bounds, and generation. Release builds may expose bounded startup/telemetry summaries; development-only USB command helpers are disabled by default and are not a configuration protocol.
