...
LedStatus LedOutputManager::configure(const std::array<LedStripConfig, board::kStripCount>& configs) {
    ...
    for (std::size_t index = 0; index < configs.size(); ++index) {
        ...
        if (!is_supported_channel_order(config.channel_order)) return LedStatus::unsupported_channel_order;
        if (config.enabled && config.pixel_count == 0) return LedStatus::invalid_pixel_count;
        if (config.pixel_count > board::kMaxPixelsPerStrip) return LedStatus::invalid_pixel_count;
    ...
    if (total > board::kMaxConfiguredPixels) return LedStatus::excessive_total_pixel_count;
    ...
    for (std::size_t index = 0; index < configs.size(); ++index) {
        ...
        if (status != LedStatus::ok) return status;
...
LedStatus LedOutputManager::initialize_drivers() {
    ...
    for (std::size_t index = 0; index < strips_.size(); ++index) {
        ...
        if (drivers_[index].usable()) {
            ...
            ++failed;
            std::printf("LED strip %u unavailable: GPIO %lu, status %u\n",
                        static_cast<unsigned>(index + 1),
                        static_cast<unsigned long>(strips_[index].config().gpio),
...
LedStatus LedOutputManager::start_show_one(std::size_t strip_index) {
    if (strip_index >= strips_.size()) return LedStatus::invalid_strip_index;
...
LedStatus LedOutputManager::start_show(uint32_t requested_strip_mask) {
    ...
    for (std::size_t index = 0; index < strips_.size(); ++index) {
        ...
        if ((requested_strip_mask & strip_mask) == 0 || !strips_[index].config().enabled ||
            !drivers_[index].usable()) {
            continue;
        }
        ...
        if (status != LedStatus::ok) {
            for (std::size_t active = 0; active < strips_.size(); ++active) {
                if (active_strip_mask_ & (1u << active)) drivers_[active].abort_transmission();
            }
            active_strip_mask_ = 0;
            phase_ = FramePhase::idle;
            return status;
        ...
        longest_pixel_count = std::max<uint16_t>(longest_pixel_count, strips_[index].config().pixel_count);
    ...
    for (;;) {
        ...
        for (std::size_t index = 0; index < strips_.size(); ++index) {
            if ((active_strip_mask_ & (1u << index)) && !drivers_[index].tx_fifo_has_word()) {
                all_preloaded = false;
                break;
            }
        }
        if (all_preloaded) break;
        if (time_us_64() >= frame_deadline_us_) return timeout_frame();
    }
    for (std::size_t index = 0; index < strips_.size(); ++index) {
        if (active_strip_mask_ & (1u << index)) drivers_[index].clear_tx_stall();
    }
    if (pio0_mask) pio_enable_sm_mask_in_sync(pio0, pio0_mask);
    if (pio1_mask) pio_enable_sm_mask_in_sync(pio1, pio1_mask);
