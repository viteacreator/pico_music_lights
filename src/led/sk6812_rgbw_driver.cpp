...
LedStatus Sk6812RgbwDriver::initialize(uint8_t strip_index, uint32_t gpio) {
  if (initialized_) {
    return strip_index_ == strip_index && gpio_ == gpio
               ? LedStatus::ok
               : LedStatus::invalid_gpio_assignment;
  }
  PIO pio = pio0;
...
bool Sk6812RgbwDriver::usable() const {
  return initialized_ && dma_channel_ >= 0;
}
...
LedStatus Sk6812RgbwDriver::claim_dma_channel() {
  ...
  std::printf("LED strip %u: PIO SM %u, GPIO %lu, DMA channel %d, DREQ %lu\n",
              static_cast<unsigned>(strip_index_),
              static_cast<unsigned>(state_machine_),
              static_cast<unsigned long>(gpio_), dma_channel_,
              static_cast<unsigned long>(dma_dreq_));
  return LedStatus::ok;
}

LedStatus Sk6812RgbwDriver::transmit_polling(const uint32_t *words,
...
LedStatus Sk6812RgbwDriver::arm_dma(const uint32_t *words,
                                    std::size_t word_count) {
  if (!initialized_)
    return LedStatus::uninitialized;
  if (dma_channel_ < 0)
...
void Sk6812RgbwDriver::abort_transmission() { reset_to_known_idle_state(); }
