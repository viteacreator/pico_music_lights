...
bool requires_state_reset(const StripEffectConfig &active,
  ...
  if ((!active.enabled && proposed.enabled) || active.type != proposed.type ||
  ...
  if (active.type == EffectType::gyver_spectrum_analyzer &&
      (active.gyver_spectrum_noise_floor !=
           proposed.gyver_spectrum_noise_floor ||
  ...
  if (active.type == EffectType::gyver_frequency_full_strip &&
      active.gyver_full_strip_selection !=
          proposed.gyver_full_strip_selection) {
    return true;
  }
...
EffectStatus EffectEngine::validate_config(const StripEffectConfig &config) {
  ...
  if (config.type == EffectType::mirrored_spectrum_zones &&
      (config.zone_count == 0u ||
       config.zone_count > kEffectMaximumMirroredZones)) {
    return EffectStatus::invalid_parameter;
  }
...
bool EffectEngine::has_pending_configuration() const { return pending_valid_; }
...
void EffectEngine::reset_state(StripEffectState &state) { state = {}; }
