...
std::array<char, config::kDiagnosticTextCapacity> g_configuration_line{};
...
storage::Rp2040FlashBackend g_configuration_flash;
storage::DeviceConfigStore g_configuration_store(g_configuration_flash);
config::ConfigService g_configuration_service;
storage::LoadResult g_configuration_boot_result{};
config::ConfigurationDiagnostics g_configuration_diagnostics{};
FirmwareConfigRuntime g_configuration_runtime{};
storage::SaveCoordinator g_save_coordinator(g_configuration_service,
                                            g_configuration_store,
                                            g_configuration_runtime,
                                            g_configuration_diagnostics);
...
void print_command_reply(const char *message) {
  const int length =
      std::snprintf(g_telemetry_line.data(), g_telemetry_line.size(),
                    "DBG command %s\n", message);
  if (length > 0 &&
      static_cast<std::size_t>(length) < g_telemetry_line.size()) {
    (void)write_usb_line(g_telemetry_line.data(),
                         static_cast<std::size_t>(length));
  }
}

uint16_t calibrated_floor(uint16_t peak) { return vu_calibrated_floor(peak); }
...
void print_vu_floors() {
  ...
  const uint16_t hysteresis =
      diagnostic_renderer_volatile_gyver_vu_hysteresis();
  const int length = std::snprintf(
      g_telemetry_line.data(), g_telemetry_line.size(),
      "DBG command vu_noise_floors measured_left=%u measured_right=%u "
      "floor_left=%u floor_right=%u hysteresis=%u mode=%s\n",
      g_vu_calibration_left_peak, g_vu_calibration_right_peak, left_floor,
      right_floor, hysteresis,
      (g_vu_calibration_last_left_floor == 0u &&
       g_vu_calibration_last_right_floor == 0u)
          ? "defaults"
          : "calibrated");
  if (length > 0 &&
      static_cast<std::size_t>(length) < g_telemetry_line.size()) {
...
void update_vu_calibration(const AudioLevelFrame &frame) {
  ...
  g_vu_calibration_left_peak =
      std::max(g_vu_calibration_left_peak, frame.left_peak);
  g_vu_calibration_right_peak =
      std::max(g_vu_calibration_right_peak, frame.right_peak);
...
void print_idle_status() {
  ...
  const effects::IdleLightingRuntime &runtime =
      diagnostic_renderer_idle_runtime();
  const int length = std::snprintf(
      g_telemetry_line.data(), g_telemetry_line.size(),
      "DBG command idle enabled=%u startup=%u state=%u mix=%u active=%u,%u,%u "
      "inactive_ms=%lu timeout_ms=%u confirm_ms=%u color=%u,%u,%u,%u "
      "brightness_q8=%u "
      "input_mask=0x%02x strip_mask=0x%02x floors=%u,%u,%u hysteresis=%u "
      "fades=%u,%u\n",
      config.enabled ? 1u : 0u, config.startup_idle_enabled ? 1u : 0u,
      static_cast<unsigned>(runtime.state), runtime.effect_mix,
      runtime.left_active ? 1u : 0u, runtime.right_active ? 1u : 0u,
      runtime.aux_active ? 1u : 0u,
      static_cast<unsigned long>(runtime.inactive_ms),
      config.silence_timeout_ms, config.audio_confirmation_ms,
      config.idle_color_rgbw.red, config.idle_color_rgbw.green,
      config.idle_color_rgbw.blue, config.idle_color_rgbw.white,
      config.idle_brightness_q8,
      static_cast<unsigned>(config.activity_input_mask),
      static_cast<unsigned>(config.strip_enable_mask),
      config.left_activity_floor, config.right_activity_floor,
      config.aux_activity_floor, config.activity_hysteresis,
      config.fade_to_effect_ms, config.fade_to_idle_ms);
  if (length > 0 &&
      static_cast<std::size_t>(length) < g_telemetry_line.size()) {
    (void)write_usb_line(g_telemetry_line.data(),
                         static_cast<std::size_t>(length));
...
void stage_idle_enabled(bool enabled) {
  ...
  if (diagnostic_renderer_stage_idle_config(config) ==
...
void handle_command_line() {
  ...
  const IdleCommandType idle_command =
      parse_idle_command(g_command_line.data());
...
void service_usb_commands() {
  ...
  for (std::size_t count = 0u;
       count < kMaximumCharactersPerLoop && tud_cdc_available() > 0u; ++count) {
...
void prepare_startup_report() {
  const int length = std::snprintf(
      g_startup_report.data(), g_startup_report.size(),
      ...
      "DBG pixels=%u,%u,%u,%u,%u,%u source=%s persisted=%u\n"
      "DBG reset_default=6x_stereo_vu_gradient diagnostic_scene=%s cycle_s=12 "
      "generation=%lu\n",
      PICO_PROGRAM_VERSION_STRING, g_renderer_available ? "ok" : "unavailable",
      ...
      g_configuration_service.active().led_channels[0].pixel_count,
      g_configuration_service.active().led_channels[1].pixel_count,
      g_configuration_service.active().led_channels[2].pixel_count,
      g_configuration_service.active().led_channels[3].pixel_count,
      g_configuration_service.active().led_channels[4].pixel_count,
      g_configuration_service.active().led_channels[5].pixel_count,
      g_configuration_service.has_persisted_record() ? "persistent" : "factory",
      g_configuration_service.has_persisted_record() ? 1u : 0u,
...
bool try_deliver_startup_report() {
  ...
  if (g_startup_report_length == 0u || !tud_cdc_connected() ||
...
void print_telemetry() {
  g_configuration_diagnostics.has_persisted_record =
      g_configuration_service.has_persisted_record();
  g_configuration_diagnostics.dirty = g_configuration_service.dirty();
  g_configuration_diagnostics.active_differs_from_last_verified =
      !config::equal(g_configuration_service.active(),
                     g_configuration_service.last_verified_persisted());
  const std::size_t configuration_length = config::format_diagnostics(
      g_configuration_diagnostics, g_configuration_line);
  if (configuration_length != 0u) {
    (void)write_usb_line(g_configuration_line.data(), configuration_length);
  }
  ...
  int length = std::snprintf(
      g_telemetry_line.data(), g_telemetry_line.size(),
      "DBG core audio_seq=%lu L=%u R=%u Aux=%u Mono=%u adc_drop=%lu "
      "adc_over=%lu "
      "adc_under=%lu fft_us=%lu fft_max_us=%lu audio_work_us=%lu "
      "audio_work_max_us=%lu "
      "dropped_windows=%lu missing_audio_blocks=%lu\n",
      static_cast<unsigned long>(g_audio_frame.sequence), g_audio_frame.left,
      g_audio_frame.right, g_audio_frame.aux, g_audio_frame.mono,
  ...
  length = std::snprintf(
      g_telemetry_line.data(), g_telemetry_line.size(),
      "DBG effects spectrum_seq=%lu bass=%u low=%u mid=%u high=%u scene=%s "
      "cfg_gen=%lu "
      "effect_us=%lu effect_max_us=%lu effect_skip=%lu effect_fail=%lu "
      "led_timeouts=%lu\n",
      static_cast<unsigned long>(g_spectrum_frame.sequence),
      g_spectrum_frame.bass, g_spectrum_frame.low, g_spectrum_frame.mid,
      g_spectrum_frame.high, diagnostic_renderer_active_scene_name(),
      static_cast<unsigned long>(
          diagnostic_renderer_configuration_generation()),
  ...
  length = std::snprintf(
      ...
      "inactive_ms=%lu input_mask=0x%02x strip_mask=0x%02x timeout_ms=%u "
      "confirm_ms=%u\n",
      idle_config.enabled ? 1u : 0u, static_cast<unsigned>(idle.state),
      idle.effect_mix, idle.left_active ? 1u : 0u, idle.right_active ? 1u : 0u,
      idle.aux_active ? 1u : 0u, static_cast<unsigned long>(idle.inactive_ms),
      static_cast<unsigned>(idle_config.activity_input_mask),
      static_cast<unsigned>(idle_config.strip_enable_mask),
      idle_config.silence_timeout_ms, idle_config.audio_confirmation_ms);
  if (length > 0 &&
      static_cast<std::size_t>(length) < g_telemetry_line.size()) {
    (void)write_usb_line(g_telemetry_line.data(),
                         static_cast<std::size_t>(length));
...
void print_vu_telemetry() {
  ...
  const uint16_t left_effective =
      g_audio_frame.left_peak > runtime.config.gyver_left_noise_floor
          ? static_cast<uint16_t>(g_audio_frame.left_peak -
                                  runtime.config.gyver_left_noise_floor)
          : 0u;
  const uint16_t right_effective =
      g_audio_frame.right_peak > runtime.config.gyver_right_noise_floor
          ? static_cast<uint16_t>(g_audio_frame.right_peak -
                                  runtime.config.gyver_right_noise_floor)
          : 0u;
  const int length = std::snprintf(
      g_telemetry_line.data(), g_telemetry_line.size(),
      "DBG vu strip=%u raw_l=%u raw_r=%u effective_l=%u effective_r=%u "
      "gate_l=%u gate_r=%u "
      "smooth_l=%u smooth_r=%u ref_l=%u ref_r=%u floor_l=%u floor_r=%u "
      "hysteresis=%u calibration=%s\n",
      static_cast<unsigned>(strip_index), g_audio_frame.left_peak,
      g_audio_frame.right_peak, left_effective, right_effective,
      runtime.left_gate_open ? 1u : 0u, runtime.right_gate_open ? 1u : 0u,
      runtime.left_smoothed, runtime.right_smoothed, runtime.left_reference,
      runtime.right_reference, runtime.config.gyver_left_noise_floor,
      runtime.config.gyver_right_noise_floor,
...
int main() {
  ...
  config::initialize_from_storage(
      g_configuration_service, g_configuration_boot_result,
      g_configuration_flash.region(), g_configuration_diagnostics);

  g_renderer_available =
      diagnostic_renderer_initialize(g_configuration_service.active());
  ...
  while (true) {
    ...
    if (audio_capture_process(g_audio_frame, g_centered_mono)) {
      ...
      if (new_spectrum) {
        ...
        g_maximum_analysis_us =
            std::max(g_maximum_analysis_us, g_spectrum_frame.analysis_time_us);
        g_spectrum_frame.maximum_analysis_time_us = g_maximum_analysis_us;
      }

      g_audio_work_us =
          static_cast<uint32_t>(time_us_64() - audio_work_start_us);
      g_maximum_audio_work_us =
          std::max(g_maximum_audio_work_us, g_audio_work_us);
